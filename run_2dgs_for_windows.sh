need_to_process=("plant5_4k" "plant6_4k" "plant7_4k") # 在这里替换为你的目标文件夹名称
# need_to_process=("car_model") # 在这里替换为你的目标文件夹名称

for work_dir in frame/*/; do
    # 获取当前文件夹名
    dir_name=$(basename "$work_dir")
    
    # 检查2dgs_gen下是否已存在同名文件夹
    # if [ -d "2dgs_gen/$dir_name" ]; then
    #     echo "Directory 2dgs_gen/$dir_name already exists, skipping..." | tee -a progress.log
    #     continue  # 跳过当前循环
    # fi
    ...
    mkdir -p "$work_dir"/* 2d-gaussian-splatting/data/input
    ...
    # 使用XMem获取完整mask
    python ./xmem/eval_test.py --model ./xmem/saves/XMem-s012.pth --generic_path ./pic --dataset G --output ./pic
