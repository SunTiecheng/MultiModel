...
for work_dir in colmap_done/*/; do
    ...
    if [[ -d "2dgs_gen/$dir_name" ]]; then
        echo "Skipping directory: $dir_name (already exists in 2dgs_gen)"
        continue
    fi
    ...
    # 运行Python脚本
    echo "Running Python script 'render.py' with output directory"
    python render.py -m output -s data
    ...
    cp -r "2d-gaussian-splatting/output" "$new_gen_dir/output"
