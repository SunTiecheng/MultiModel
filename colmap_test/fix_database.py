...
class COLMAPDatabase(sqlite3.Connection):
    ...
    def update_camera(self, model, width, height, params, camera_id):
        ...
        if cursor.rowcount == 0:
            print(f"No rows updated for camera_id: {camera_id}")
        else:
...
def camTodatabase(txtfile):
    ...
    print("Connecting to database:", args.database_path)
    ...
    with open(txtfile, "r") as cam:
        ...
        for i in range(0,len(lines),1):
            if lines[i][0]!='#':
                ...
                # 输出解析的内容，确保解析正确
                print(
                    f"Camera ID: {cameraId}, Model: {cameraModel}, Width: {width}, Height: {height}, Params: {params}")
